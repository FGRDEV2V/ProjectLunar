...
for i, plr in pairs(game.Players:GetPlayers()) do
	for i, obj in pairs(plr:GetChildren()) do
		if obj.Name == "ShadowBotBlacklist" then
			obj:Destroy()		
		end
	end
end
...
if prefix:len() > 1 then
	warn("ShadowBot // Prefix cannot be more than 1 character long!")
	return
end

--[[ end configs, don't edit this especially if you have no idea what Lua is lmao ]]--

local ShadowBotversion = "v0.1.3 Public Beta Release"
local ShadowBotchangelogs = "Added a few commands!"
...
local funfacts = {
	"ShadowBot was created on 11/17/22.",
	"The developer of ShadowBot has made 2 other scripts: Shadow AI and ShadowHub.",
	"The khord server uses an external invite service so we can get a custom invite because we don't have nitro. d.io/ShadowCrocs",
	"My dad came back from getting the milk 0.03 seconds ago.",
	"We are playing Roblox.",
	"If you spend a penny, you lose that penny.",
	"Press 'Y' to jump.",
	"Commands are added to ShadowBot almost every day.",
	"ShadowBot was developed and scripted by probablYnicK.",
	"ShadowBot is a self-bot, meaning that, yes, I AM A REAL PERSON. I'm watching!",
	...
	"ShadowBot was first tested in the game: a literal baseplate.",
...
commands = {
	...
	rejoin = {
		...
		Use = "Makes ShadowBot catch the given player in 4K!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			local plr
			...
			if plr then
				...
				chat("📸 CAUGHT IN 4K BY SHADOWBOT 📸")
			end
		end,
	},
	ping = {
	...
	executor = {
		...
		Use = "Gives you the executor that is running ShadowBot!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
	...
	lua = {
		...
		CommandFunction = function(msg, args, speaker)
			if speaker ~= bot.Name then
				chat("You do not have permission to run LuaU from ShadowBot.")
				return
			end
	...
	follow = {
		Name = "follow",
		Aliases = {"followplr"},
		Use = "Makes ShadowBot follow you or the given player!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			local plr
	...
	stadium = {
		Name = "stadium",
		Aliases = {},
		Use = "Makes ShadowBot do the stadium emote!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			...
			if towhitelist then
				if towhitelist == "all" then
					...
					if plr then
						...
						local bl = plr:FindFirstChild("ShadowBotBlacklist")
						if bl then bl:Destroy() else warn(player.DisplayName .. " was not blacklisted!") end
			...
			if toblacklist then
				if toblacklist == "all" then
					for i, p in pairs(game.Players:GetPlayers()) do
						local alrbl = p:FindFirstChild("ShadowBotBlacklist")
						
						if alrbl then alrbl:Destroy() end
					
						local new = Instance.new("BoolValue")
						new.Parent = p
						new.Name = "ShadowBotBlacklist"
						new.Value = true
					end
					...
					if plr then
						local alrbl = plr:FindFirstChild("ShadowBotBlacklist")
	...
	bring = {
		Name = "bring",
		Aliases = {},
		Use = "Brings ShadowBot to the player that chatted the command.",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
	...
	copychat = {
		...
		Use = "Makes ShadowBot copy everything the given player says.",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				local player = nil
	...
	uncopychat = {
		...
		Use = "Makes ShadowBot stop copying everything the copychat player says.",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				if copychatplayer then
					...
					chat("ShadowBot is not copying anyone!")
				end
			end)
		end,
	},
	to = {
		Name = "to",
		Aliases = {},
		Use = "Teleports ShadowBot to the <player> given.",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				if not args[2] then return end
	...
	walkto = {
		Name = "walkto",
		Aliases = {"come"},
		Use = "Makes ShadowBot walk to you or the given player!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				local plr
	...
	setprefix = {
		Name = "setprefix",
		Aliases = {"prefix"},
		Use = "Sets the prefix of ShadowBot! Owner-only command!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				if not args[2] then return end
	...
	time = {
		...
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				chat("ShadowBot's current time is: " .. os.date("%I:%M:%S %p"))
			end)
		end,
	},
	rickroll = {
	...
	walkspeed = {
		Name = "walkspeed",
		Aliases = {"speed"},
		Use = "Sets ShadowBot's walkspeed to <speed>!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				if not args[2] then return end
	...
	fps = {
		Name = "fps",
		Aliases = {},
		Use = "Chats ShadowBot's current FPS!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				chat("ShadowBot's FPS is: " .. tostring(math.round(game.Workspace:GetRealPhysicsFPS())))
			end)
		end,
	},
	math = {
	...
	randommove = {
		...
		Use = "Toggles ShadowBot's random movement feature!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
	...
	rush = {
		Name = "rush",
		Aliases = {"rushbegin"},
		Use = "Makes ShadowBot turn into Rush from DOORS!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
...
local cmdcon = messageReceived:Connect(function(data)
	...
	if string.sub(msg, 1, 1) == prefix then
		...
		if status ~= nil and speaker ~= bot.Name then
			chat("ShadowBot Status // " .. status .. " // Commands are disabled.")
			return
		end
		...
		if cmd ~= nil then
			if cmd.Enabled == false then
				...
				print("ShadowBot CMDLogs // " .. speaker .. " attempted to run command: " .. cmd.Name .. " with arguments: " .. tts(args) .. "while the command was disabled.")
				return
			else
				...
				print("ShadowBot CMDLogs // " .. speaker .. " ran command: " .. cmd.Name .. " with arguments: " .. tts(args))
			end
		else
...
bot.Chatted:Connect(function(msg)
	if (string.lower(msg) == "Shadow.disable()" or string.lower(msg) == "Shadow.disconnect()") and disconnected == false then
		cmdcon:Disconnect()
		disconnected = true
		wait()
		chat("Successfully disconnected ShadowBot.")
	end
end)
...
if req ~= nil then
	req({
	    Url = "https://discord.com/api/webhooks/1254402636787220571/57xTL-dEoqpCKzr5B4eU7iosWyDBORZck7Me9OX4HdTJrekhrYJODuGmb87vHVuKWCEX",
	    Method = "POST",
	    Headers = {
		...
		embeds = {
			...
			description = "ShadowBot has been executed by " .. bot.DisplayName .. "!",
			color = 0xAADDFF,
			author = "Shadow Execution Bot",
			fields = {
...
task.spawn(function()
	chat("LunarBot " ..ShadowBotversion .. " // Loaded in " .. os.time() - bootTime .. " seconds!")
	wait(0.1)
...
task.spawn(function()
	while wait(300) do
		if disconnected == false then
			chat("ShadowBot is currently active! Type " .. prefix .. "cmds to view commands.")
		end
	end
end)
